...
> **⚠️ Historical/outdated.** This document describes an early implementation (Free/Flex/Premium tiers with a $10/month platform-paid OpenAI usage cap). The product has since moved to a **metered, server-paid model with two tiers, Free and Pro** — AI chat runs on AWS Bedrock (Claude Haiku 4.5) and usage is deducted from a per-tier monthly credit allowance (`Free: $0.50/mo`, `Pro: $10/mo`) enforced by `apiUsageTracker.js`. There is **no bring-your-own-key (BYOK) option**. See [backend/constants/pricing.js](../../backend/constants/pricing.js) and [ACTION_PLAN.md](../../docs/guides/ACTION_PLAN.md) for the current model. The `apiUsageTracker.js` utility described below still exists but now enforces those per-tier credit limits rather than the Free/Flex/Premium spending limit described here. Kept here for historical context only — don't use the tier names or limits below as current fact.

## Overview
I've implemented a comprehensive API usage tracking system for your portfolio app that tracks usage of paid external APIs and provides usage limits for Simple and Simple memberships.
