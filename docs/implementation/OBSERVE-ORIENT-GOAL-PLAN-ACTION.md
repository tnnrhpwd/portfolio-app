...
### 2026-09-06 — Chat-driven logic mode (loop as the chat's default front door)

- **New feature (makes §6.4 "chat as the front door" real):** the webapp chat now defaults to **logic mode** — every plain-text message is tried through the O-O-G-P-A loop first, and falls back to normal conversational chat only when the loop judges it non-actionable. A message like *"List the files in `controllers`, count them, and report the count."* creates a goal, runs the loop, and reports the final count back into the conversation.
- **Addon** (`automation/index.js`): new `POST /api/agent/run` — `classifyActionable()` (action/chat keyword heuristic → LLM verdict for the ambiguous middle) → create goal → run loop to completion → return `{ actionable, goalSlug, status, result, steps, reason }`. Non-actionable messages short-circuit **before** any goal is created or tool runs.
- **Loop** (`agent-loop.js`): on `<<GOAL_DONE>>` the final answer is now captured (`state.finalAnswer`), the goal is persisted `done`, and `agent.reply` + `goal.done` events are published; `status()` exposes `finalAnswer`.
- **Cloud relay** (`cloud-relay.js` + `backend/controllers/addonRelayController.js`): new `agent_run` command type so a remote device (phone → cloud → desktop) drives the same loop; in-flight command dedupe prevents re-delivery during long runs.
- **Frontend** (`SimpleChat.jsx` + `simpleAddonApi.js`): `runAgentMessage()` (local `/api/agent/run` or remote `agent_run` relay) is the new default message path — actionable messages render the final answer with a "(ran N steps)" note; non-actionable ones fall through to the normal provider.
- **Tests:** `agent-loop.test.js` 29 cases (adds finalAnswer + `agent.reply`/`goal.done` assertions); eval scenario `25-agent-run-nonactionable.json` (offline `actionable:false` fast path) → `runner.test.js` 35 cases. Full `test:unit` green; `node --check` clean; frontend `tsc --noEmit` clean.

### 2026-09-06 — Continuous listener (the always-on feedback loop)

- **New feature — §6.3 `continuous` autonomy level, now shippable because Phase 6 (idle/stall/kill-switch) landed.** A `ContinuousListener` (`automation/listener.js`) runs on a 60s cadence and autonomously keeps work moving: (1) if every loop is idle and an active goal waits, start one; (2) analyze the pattern learner and auto-promote a **high-confidence, non-destructive** suggestion into a goal, then start the loop on it.
- **Safety (hard stops):** the kill switch blocks every tick; only `safe-read`/`sandboxed-write` suggestions are ever auto-started (anything riskier is left for the human); per-goal (10 min) and per-`sequenceKey` cooldowns prevent re-starting the same work in a tight loop; auto-created goals carry `autoAbandon: true` so a stalled auto-goal self-blocks. Off by default.
- **Toggle surfaces (all three):** dashboard Agent tab checkbox, webapp `AgentLivePanel` "Listener" toggle, and chat `/agent listen on|off`. State persists via the permission store (`continuousMode`, new `DEFAULTS` key).
- **Endpoints:** `GET`/`POST /api/agent/listener` (`automation/index.js`); listener is a module-level singleton disposed on automation remount so re-mounts never leak timers.
- **Tests:** new `listener.test.js` (9 cases — enable/disable persistence, kill-switch gate, idle-start cooldown, non-destructive filter, suggestion auto-start + dedupe) registered in `test:unit`. Full suite green; `node --check` clean; frontend `tsc --noEmit` clean; DASHCHECK clean (script exit 0, tags balanced).

### 2026-09-06 — Goal formation (self-formed goals + surfaced proposals)

- **Extends the continuous listener with a Goal-formation stage** — relaxing the plan's "goal formation stays human-in-the-loop" guardrail by user request, but bounded to non-destructive + kill-switch gated. On a 5-min cadence, `_maybeFormGoals()` asks the LLM to propose **new** goals from the current situation (active goals, recent actions, latest perception frame).
- **Self-forming:** non-destructive, high-confidence (≥0.7) proposals are auto-created as goals and the loop starts on them immediately (`createdBy: 'listener-formed'`) — the "make goals themselves and work them continuously" behaviour.
- **Surfaced suggestions:** every proposal (including destructive / low-confidence) is stored and exposed for manual accept — `GET /api/agent/proposed` + `POST /api/agent/proposed/accept` (manual accept creates + starts *any* proposal, since the user explicitly approved it). Deduped by title+description hash; capped at 10.
...
### 2026-09-06 — /plans blend (Phase 1) + agent context

- **Phase 1 bridge:** the `/plans` goal page now drives the **addon O-O-G-P-A loop** instead of the old server-side `goalAgentService`. `GoalDetail.jsx` calls `runAgentMessage()` (local `/api/agent/run` or remote `agent_run` cloud relay) and renders the final answer ("🤖 Agent result (N steps)"). The list page's "Enlist agent" now routes to the goal page (where enlistment happens). The old `data.agent` feed still renders for historical runs; the server-side `goalAgentService` remains the offline fallback path for later.
- **Agent context:** `runAgentMessage` + `runGoalToCompletion` + `POST /api/agent/run` now thread an optional `context` string, folded into the goal `content` as a `CONTEXT / SCOPE:` block so the loop's Orient/Plan sees the user's scope clarification and "agentic implementation" instructions. `GoalDetail` adds a "🧭 Scope / instructions" textarea.
- **Safety unchanged:** classification still keys on the description only (context is guidance, not the actionable trigger); non-actionable goals still short-circuit before any goal is created or tool runs.
- **Validation:** `node --check` clean; frontend `tsc --noEmit` clean (incl. `noUnusedLocals` after removing the now-unused `startGoalAgent` import); full `test:unit` green (runner 35/35).
