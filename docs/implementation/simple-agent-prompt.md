...
Give this document to your coding agent (Claude Code, etc.) as the starting spec. It expands on an existing codebase (`simple-addon/`) rather than starting from scratch — read the existing code first before proposing changes.
...
5. [Skill generalization](#5-skill-generalization-priority-ordered-technical-roadmap) — 🟡 partial (5.1/5.2/5.5/5.6 shipped)
6. [Safety & permissions](#6-safety--permissions) — 🟡 partial (backend seams shipped, UI open)
7. [Architecture constraints](#7-architecture-constraints--stack-constraints-already-honored-llm-provider-seam-71-now-shipped-for-core-callers) — ✅ 7.1 seam now shipped for every LLM-calling caller
8. [Monetization](#8-monetization--planned-no-requiresplanstripe-wiring-in-simple-addon-yet) — ⬜ planned
...
> ✅ **Implementation update (2026-08-13):** GitHub Models (`models.github.ai`) was permanently retired by GitHub on 2026-07-30, which broke every §7.1 caller listed below (they all ultimately bottomed out in `server/github-models-service.js`, which called `models.github.ai` directly using a per-user GitHub PAT). This has been replaced end-to-end:
> - `server/github-models-service.js` is **deleted**. There is no more per-user GitHub PAT setting anywhere in the addon or frontend (`githubToken`/`githubModel` fields removed from `simple-addon`'s webapp settings, `frontend/src/components/SimpleAddon/*`, and `frontend/src/pages/Settings/Settings.jsx`).
> - `server/automation/llm-provider.js`'s `createLlmProvider()` default adapter is now **`backend-proxy`** (was `github-models`). It proxies every `.chat()`/`.chatMultimodal()`/`.chatWithImage()`/`.chatRaw()` call through the portfolio backend's `/api/data/csimple/agent-chat` and `/api/data/csimple/agent-vision` routes (new — see `backend/controllers/workspaceController.js`'s `agentChatProxy`/`agentVisionProxy`), authenticated with the user's existing JWT (via `workspace-client.js`'s already-wired cloud-relay token getter — no new plumbing needed). `setToken()` is now an interface-compat no-op / explicit-override hook, not a required call.
> - **Why this had to be backend-only**: the backend calls AWS Bedrock (Claude Haiku 4.5) using the app operator's own AWS IAM credentials. Unlike the old per-user GitHub PAT (safe client-side, since it was each user's own free credential), AWS IAM secrets belong to the operator's account — embedding or exposing them to a distributed desktop addon would let any user extract them and access/bill the operator's entire AWS account (including DynamoDB). So the addon now ALWAYS proxies through the backend and never attempts a "local direct" LLM call first.
> - `nl-compiler.js`'s `compile()`/`editSteps()` no longer do the local GitHub-token-discovery dance (inline token → `readWebappSettings()` → `settings.json` DPAPI read) — production calls (no injected `llmClient`) go straight to `workspace-client.js`'s `compileNaturalViaBackend()`/`editNaturalViaBackend()`. `server/automation/index.js`'s `/api/skill/compile-natural`/`/api/skill/edit-natural` routes lost their "try local, fall back to backend on token error" dance for the same reason — there's no more "local" path to fall back from.
> - `backend/controllers/workspaceController.js`'s `compileMacroNatural`/`editMacroNatural` (the endpoints the addon proxies to) no longer require a per-user GitHub PAT either — they call the same Bedrock-backed `_callAgentLlm()` helper as the two new routes above.
> - Tests updated: `llm-provider.test.js` and `vision-fusion.test.js` now fake `workspace-client.js` (the addon's one remaining network dependency) instead of `github-models-service.js`. Eval scenario `12-nl-compile.json` updated to describe the new architecture (still gated behind `EVAL_ALLOW_LLM=1`, still just needs a signed-in JWT instead of a GitHub token).
...
- ✅ Section 7.1 (LLM provider seam) now shipped for every LLM-calling caller in `simple-addon`: `server/automation/llm-provider.js` (`createLlmProvider()`) is used by `agent-loop.js`, `nl-compiler.js`, `tools/skill.js` (both call sites), and now also `vision-fusion.js`/`tools/webcam.js` — no module in the addon instantiates `GitHubModelsService` directly anymore. A local-stub adapter + centralized retry policy are shipped and available (opt-in) to any caller.
...
### Implementation update (2026-07-16, continued 4)

- ✅ §7.1 fully closed: `vision-fusion.js`'s `find_and_click_visual` and `tools/webcam.js`'s `_describeFrame` now both call `createLlmProvider()` (`server/automation/llm-provider.js`) instead of directly `require`-ing and `new`-ing `GitHubModelsService`. Same pure instantiation-seam refactor as the three originally-checklisted callers — no method-call-shape changes at either site, so `vision-fusion.test.js`'s `require.cache`-based `GitHubModelsService` stub keeps working unmodified (verified: `vision-fusion.test.js` 3/3 and `llm-provider.test.js` 9/9 still pass, plus the full `test:unit` suite). Every LLM-calling module in `simple-addon` now goes through the single provider factory; §7.1's checklist item "Route `agent-loop.js`, `nl-compiler.js`, and `tools/skill.js` through the provider factory" is superseded by full coverage of every caller, including the two previously-unmigrated ones.
- ✅ §5.5 CI gate shipped, closing its last open checklist item: `.github/workflows/ci.yml` gained a `test-simple-addon` job (`windows-latest`, matching `build-addon.yml`'s runner since the automation tools shell out to PowerShell/UIA) that runs `npm ci`, `npm run test:unit`, and `npm run eval` on every push/PR to `master`. `npm run eval` (`server/automation/eval/cli.js`) already exits non-zero on any scenario failure, so wiring it into CI is what makes "no regression in scenario pass rate" an enforced gate rather than a manually-run local check. Verified locally: `npm run test:unit` (all suites, including `vision-fusion`/`llm-provider`) and `npm run eval` (20 scenarios: 19 passed, 1 skipped via the existing `EVAL_ALLOW_LLM` env gate, 0 failed) both pass clean against the current tree.

## 1. Vision

Turn the existing `simple-addon` (currently a personal/dev-focused Electron+Node addon) into a **consumer-facing PC automation platform** where any user can:
...
## 5. Skill generalization (priority-ordered technical roadmap) — 🟡 partially implemented (5.1/5.2/5.5/5.6 shipped; 5.3/5.4 partial)
...
### 5.5 Measure it — reuse the eval harness — ✅ implemented 2026-07-16
...
- ✅ Add CI gate requiring no regression in scenario pass rate for `automation/eval/scenarios/` (`.github/workflows/ci.yml`'s new `test-simple-addon` job runs `npm run test:unit` + `npm run eval` on every push/PR to `master`; `npm run eval` already exits non-zero on any scenario failure, so wiring it into CI is what makes this an enforced gate — see §5.5's "Implementation update (2026-07-16, continued 4)" below).

> ✅ **Implementation update (2026-07-16, continued 4):** §5.5 is now fully closed. `.github/workflows/ci.yml` gained a `test-simple-addon` job (`windows-latest`, matching `build-addon.yml`'s runner) that runs `npm ci && npm run test:unit && npm run eval` in `simple-addon/` on every push/PR to `master`. No new scoring logic was needed — `server/automation/eval/cli.js` already fails the process (exit 1) the moment any scenario in `automation/eval/scenarios/` regresses from pass to fail; the gap was purely that nothing in CI ever ran it. Verified locally against the current tree: 20 scenarios, 19 passed, 1 skipped (the `EVAL_ALLOW_LLM`-gated live-LLM scenario), 0 failed.
...
The generalization and vision-fallback paths send **screen captures to a cloud multimodal LLM** (`vision-fusion.js`, proxied through the backend to AWS Bedrock — Claude Haiku 4.5 — as of 2026-08-13). For a consumer product that's a real data-egress concern the current dev tool glosses over. Require explicit, revocable consent before any screen frame leaves the device, show which paths use it, and make the local/offline model seam (Section 7) the escape hatch for privacy-sensitive users. This consent is separate from the keyboard-capture consent in Section 6.1.
...
- **Cloud-first AI, with a local/offline option**: default to cloud APIs (AWS Bedrock — Claude Haiku 4.5 — proxied through the portfolio backend; see the 2026-08-13 update above) for the "smart" parts (NL compilation, skill generalization, vision fallback) since that's simplest to ship, but preserve the ability to swap in local models later — don't hardcode cloud-only assumptions into new code. Concretely: define a small **LLM provider interface** (`chat`, `chatMultimodal`) and route `agent-loop.js`, `nl-compiler.js`, and `tools/skill.js` through it instead of newing up a provider client directly, so a local backend can be dropped in behind the same seam.
...
### 7.1 LLM provider seam implementation checklist — ✅ implemented 2026-07-16

> ✅ **Implementation update (2026-07-16, continued 4):** the seam is now fully closed. `vision-fusion.js`'s `find_and_click_visual` and `tools/webcam.js`'s `_describeFrame` — the two callers explicitly called out below as the "one remaining gap" — now also call `createLlmProvider()` instead of directly `require`-ing and `new`-ing `GitHubModelsService`. Same pure instantiation-seam refactor as the three originally-checklisted callers: no method-call-shape changes, `vision-fusion.test.js`'s `require.cache`-based stub keeps working unmodified, and the full `test:unit` suite (including `vision-fusion.test.js` and `llm-provider.test.js`) passes clean. Every LLM-calling module in `simple-addon` now goes through the single `llm-provider.js` factory.
...
- ✅ Route `agent-loop.js`, `nl-compiler.js`, and `tools/skill.js` through the provider factory (all three done; `vision-fusion.js`/`tools/webcam.js` — mentioned only in prose, not originally in this checklist — are now ALSO routed through the seam, see the 2026-07-16 continued 4 update above).
...
- ✅ Add unit tests proving callers no longer instantiate `GitHubModelsService` directly (proven for all callers, including `vision-fusion.js`/`tools/webcam.js`, via `llm-provider.test.js` + `vision-fusion.test.js` + full `test:unit` regression pass).

## 8. Monetization — ⬜ planned (no `requiresPlan`/Stripe wiring in `simple-addon` yet)
...
1. ✅ **Generalization MVP** — LLM re-derivation (5.1) + multi-demo parameter inference (5.2) shipped.
   - ✅ Eval harness now supports an HTTP scenario mode (5.5) with four previously-inert scenarios now live, plus all three perturbed-UI regression axes for `inferParams` (position-shift, label-rename, timing-variance — scenarios `18`/`19`/`20`); the CI pass-rate gate (`.github/workflows/ci.yml`'s `test-simple-addon` job) now also runs it on every push/PR, fully closing 5.5.
...
- 🟡 LLM provider seam extraction + local adapter quality pass (§7.1 core seam now shipped for ALL callers — `agent-loop.js`/`nl-compiler.js`/`tools/skill.js`/`vision-fusion.js`/`tools/webcam.js` — plus a deterministic local-stub adapter; only a real "quality" local adapter, e.g. an actual local model backend rather than the current echo-only stub, remains open).
