...
- **Part 3 — Sprite sheets → individual sprites**: generating a white-background
  ...
  cropped sprites — the full pipeline, the spec, the review workflow, the gotchas.
- **Part 4 — Uploads & storage in the app**: what happens when the *app* (not you)
  writes to the bucket, and how each of those paths broke.
- **Part 5 — Music & SFX, rendered offline**: a dependency-free chiptune engine
  that renders a game soundtrack from a spec — and why AI audio was ruled out.

Use Part 1 for "where do I put an image and how do I serve it?", Part 2 for
"how do I generate a new image for a project card and check it in?", Part 3 for
"how do I turn one AI image full of little drawings into a usable sprite set?", and
Part 5 for "where does the music come from?".
...
The full walkthrough is in the *Review workflow* and *Gotchas* sections below. The short
...
## Part 4 — Uploads & storage in the app (and how they broke)
...
### Eleventh audit pass — uploads, storage, and the bugs they hid (2026-09-12)

Surfaced while building the Dream board ([`GOALS.md`](../implementation/GOALS.md) → *Dream board*), which is the first frontend feature
...
## Part 5 — Music & SFX, rendered offline

The audio counterpart to Part 3, and the same shape: a **spec is the source of truth**,
a script is the mechanics, and the game reads only the generated manifest.

Rocket's music and sound effects are synthesized offline by `scripts/rocket/generate-audio.js` — a small chiptune engine (oscillators, ADSR
envelopes, a filter, echo, drum synthesis, a step sequencer) with **no dependencies,
no accounts, no network and no per-second bill**. Output is real audio files in
`frontend/public/rocket/audio/` plus a `manifest.json` the game reads at runtime.

- **Read this** before changing a soundtrack, adding a track, or wiring audio into a
  scene. Scene-side behaviour lives in `game/audio/music.ts`.
- The game's *effects* are a separate, older layer of runtime WebAudio synthesis
  (`game/audio/sfx.ts`); this Part is about rendered assets.
...
- `scripts/rocket/extract-sprites.js`, `scripts/rocket/sheets.json`,
  `scripts/rocket/qa-composite.js` — the sprite pipeline itself (Part 3).
- `scripts/rocket/generate-audio.js`, `scripts/rocket/audio.json` — the audio
  renderer and its spec (Part 5).
