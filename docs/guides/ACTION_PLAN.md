...
- [ ] **Disable the ability to purchase Pro** (checkout/payment flow) while Phase 0 is unresolved — a temporary, explicit "not yet available" state is more honest than a working checkout for a non-working product.
- [ ] **Hide or clearly caveat CSimple mentions and entry points** (Pricing page CSimple content, download prompts, homepage mention from Phase 1) until the readiness bar below is met, or add a plain "this is early and not fully working yet" notice if you'd rather keep it visible during development.
