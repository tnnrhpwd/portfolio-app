# Operations — deployment, secrets, traffic

How the app is deployed, where its configuration and secrets come from, and what
traffic analytics exist. Three subjects that used to be three short docs: the
deployment half is the map, the secrets half is the one workflow that touches every
environment, and the analytics half is the referer tracking the admin console reads.
...
Secrets Manager. See *Secrets & configuration* below.
...
**AWS Secrets Manager is the single source of truth for secrets *and* app config.**
The three parts below are the workflow, the offline `.env` backup, and a worked
example of adding a provider key end-to-end.
...
## Traffic analytics (HTTP referer tracking)

The referer tracking behind the admin console's visitor map and the page-view
counters: what the `checkIP` middleware records, how referers are categorised, and
the analytics endpoints that read it.
...
- [`ASSETS.md`](./ASSETS.md) — the asset pipelines and the S3/CloudFront setup.
- [`.github/workflows/`](../../.github/workflows/) — CI, which is where a deploy actually starts.
