...
function _downscaleToJpeg(pngBuffer, maxDim, quality) {
    return new Promise((resolve, reject) => {
        const inFile  = path.join(os.tmpdir(), `simple-relay-in-${Date.now()}-${Math.floor(Math.random()*1e6)}.png`);
        const outFile = path.join(os.tmpdir(), `simple-relay-out-${Date.now()}-${Math.floor(Math.random()*1e6)}.jpg`);
