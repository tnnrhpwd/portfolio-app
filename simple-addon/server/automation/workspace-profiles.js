 ...
 * msedgewebview2 process) hung/crashed Simple Addon's own window instead
...
/**
 * Simple Addon's own process name (e.g. "Simple Addon" in the packaged
 ...
 * where Explorer itself survived but Simple Addon's own window vanished.
 */
    ...
    for (let i = 0; i < profile.windows.length; i++) {
        ...
        try {
            ...
            if (entry.processName === OWN_PROCESS_NAME) {
                skipped.push({ processName: entry.processName, title: entry.title, reason: 'Simple Addon\'s own window — never repositioned to avoid desyncing Electron\'s window state' });
                continue;
            }
