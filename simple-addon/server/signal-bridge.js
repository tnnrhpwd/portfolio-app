 ...
 * LLM-provider seam (`LLM_PROVIDERS.md`) and defaults SIGNAL_MODEL to gpt-4o-mini. Only a
...
const CONFIG_DIR = path.join(os.homedir(), 'Documents', 'Simple', 'Resources');
...
class SignalBridge {
  ...
  async run() {
    console.log('\n════════════════════════════════════════════════════════════');
    console.log('  🔗 Simple Signal Bridge');
    console.log('════════════════════════════════════════════════════════════');
...
async function runSetup() {
  ...
  if (method.trim() === '2') {
    ...
    try {
      ...
      const output = linkCli.exec(['link', '-n', 'Simple-Bot'], 60000);
      console.log(`\nLink URI:\n${output}`);
...
async function linkDevice() {
  ...
  try {
    ...
    const cmd = isBat
      ? `cmd /c ""${config.signalCliPath}" link -n "Simple-Bot""`
      : `"${config.signalCliPath}" link -n "Simple-Bot"`;
