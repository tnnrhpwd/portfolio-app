...
// Windows Virtual Key Codes (matches Simple KeyCodes.cs VirtualKey enum)
const VK = {
  BACK: 0x08,
  TAB: 0x09,
  RETURN: 0x0D,
...
class ActionService {
  constructor() {
    ...
    this.WORKSPACE_PATH = path.join(os.homedir(), 'Documents', 'Simple', 'Workspace');
    ...
    this.HISTORY_PATH = path.join(os.homedir(), 'Documents', 'Simple', 'Resources', 'action-history.json');
    // Ensure dirs exist
    const fs = require('fs');
  ...
  async executeScript(filename, args = []) {
    ...
    return new Promise((resolve) => {
      ...
      const child = spawn(command, commandArgs, {
        ...
        timeout: TIMEOUT_MS,
        env: { ...process.env, SIMPLE_WORKSPACE: this.WORKSPACE_PATH },
        windowsHide: true,
      });
