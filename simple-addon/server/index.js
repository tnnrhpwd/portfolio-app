...
// Simple Resources path — reads from global (set by main.js) or falls back to default
function resolveResourcesPath() {
  ...
  try {
    const configPath = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'simple-addon', 'resources-path.json');
...
// Sandboxed file operations in ~/Documents/Simple/Workspace/
...
app.get('/api/workspace/preview/:filename', (req, res) => {
  const path = require('path');
  const fs = require('fs');
  const workspaceRoot = path.join(require('os').homedir(), 'Documents', 'Simple', 'Workspace', 'files');
...
app.get('/api/signal/config', (req, res) => {
  const configPath = path.join(os.homedir(), 'Documents', 'Simple', 'Resources', 'signal-bridge.json');
  if (fs.existsSync(configPath)) {
    try {
      const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      const safe = {
...
function ensureSelfSignedCert() {
  ...
  try {
    ...
    execSync(
      ...
      `-days 365 -nodes -subj "/CN=Simple Local Dev" ` +
...
function startServer(options = {}) {
  ...
  // hostBinding to 'lan' in the Permission Center (or set SIMPLE_BIND_HOST).
