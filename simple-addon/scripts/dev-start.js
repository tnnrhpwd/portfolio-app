/**
 * dev-start.js — kills any running Simple Addon instance, then starts Electron.
 *
 * Run via:  npm start   (triggered by prestart hook)
 *           npm run dev (explicit)
 *
 * Why this exists:
 *   The packaged installer puts a copy of the code in %LOCALAPPDATA%\Programs\Simple Addon\.
...
// ─── Kill any running Simple Addon process ────────────────────────────────────
// Includes legacy "CSimple" exe names so dev-start still kills old installed
// versions during the CSimple -> Simple transition period.
const targets = ['Simple Addon.exe', 'simple-addon.exe', 'SimpleAddon.exe', 'CSimple Addon.exe', 'csimple-addon.exe', 'CSimpleAddon.exe'];
let killed = false;
