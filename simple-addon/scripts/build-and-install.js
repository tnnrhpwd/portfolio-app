 ...
 *
 * This replaces the old packaged version in %LOCALAPPDATA%\Programs\Simple Addon\
 * so that:
...
console.log('[build:install] Stopping any running Simple Addon instance...');
for (const name of ['Simple Addon.exe', 'simple-addon.exe']) {
...
const expectedName = `Simple Addon Setup ${pkg.version}.exe`;

let setupInstallers = fs.readdirSync(distDir)
    .filter(f => /^Simple Addon Setup .*\.exe$/.test(f))
    .map(f => ({ name: f, mtime: fs.statSync(path.join(distDir, f)).mtimeMs }));

if (setupInstallers.length === 0) {
    console.error('[build:install] No "Simple Addon Setup *.exe" installer found in dist/');
    process.exit(1);
}
...
const appExe = path.join(
    ...
    'Programs', 'Simple Addon', 'Simple Addon.exe'
);
