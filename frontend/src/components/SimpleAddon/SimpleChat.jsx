...
const DEFAULT_SETTINGS = {
  ...
  agents: [{ id: 'default', name: 'Simple AI', behaviorFile: 'default.txt', avatarUrl: null, voiceURI: '' }],
...
function SimpleChat({
  ...
  // Apply Simple theme to the container element (not document root)
  useEffect(() => {
  ...
  useEffect(() => {
    if (rootRef.current) {
      rootRef.current.setAttribute('data-simple-font-size', settings.fontSize || 'medium');
    }
  }, [settings.fontSize]);
  ...
  useEffect(() => {
    ...
    if (!file || file === 'none') {
      setBehaviorContent('You are a helpful AI assistant powered by Simple. Answer clearly and concisely.');
      return;
    }
    if (isAddonConnected) {
  ...
  useEffect(() => {
    ...
    const checkAndLoadCloud = async () => {
      try {
          ...
          if (cloudSettings.cloudSync) {
            ...
            if (isAddonConnected) {
              ...
              saveAddonSettings(toAddon).catch(err => {
                console.warn('[Simple] Failed to push cloud settings to addon:', err);
              });
            }
        ...
        console.warn('[Simple] Cloud sync initial load failed:', err);
        setCloudSyncStatus('error');
      }
  ...
  useEffect(() => {
    ...
    cloudSettingsTimer.current = setTimeout(async () => {
      try {
        ...
        console.warn('[Simple] Cloud settings sync failed:', err);
        setCloudSyncStatus('error');
      }
    }, 500);
  ...
  const runRemoteRelay = useCallback(async (message, history, model) => {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      if (poll.status === 'error') {
        throw new Error(friendlyRemoteError(poll.error) || 'Remote addon execution failed');
      }
  ...
  const runRemoteConfirm = useCallback(async (confirmationId, selectedOption) => {
    ...
    for (let i = 0; i < MAX_POLLS; i++) {
      ...
      if (poll.status === 'error') throw new Error(friendlyRemoteError(poll.error) || 'Remote confirmation failed');
    }
  ...
  useEffect(() => {
    ...
    if (lastMsg && lastMsg.isError && lastMsg.id !== lastReportedRef.current) {
      ...
      const bugData = {
        text: [
          `Bug:Simple Auto-Report — LLM Error`,
          `Severity:high`,
          `Description:${truncated}`,
          `Steps:Chatting with Simple AI`,
  ...
  return (
    <div className="simple-root" ref={rootRef}>
        ...
        {isInactive && (
          <div className="simple-inactive-overlay" onClick={resumeActivity}>
            <div className="simple-inactive-overlay__content">
              ...
              <div className="simple-inactive-overlay__title">Tab Paused</div>
              <div className="simple-inactive-overlay__subtitle">
                Mic listening paused to save resources
              </div>
              <button className="simple-inactive-overlay__btn" onClick={resumeActivity}>Click to Resume</button>
            </div>
          </div>
        )}
      </div>
