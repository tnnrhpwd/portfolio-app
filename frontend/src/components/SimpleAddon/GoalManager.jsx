...
async function fetchSuggestions() {
  try {
    const { addonFetch } = await import('../../services/simpleAddonApi');
    // addonFetch is not exported directly — use the public helper instead
    const res = await fetch(`${window._simpleAddonBase || 'http://localhost:3001'}/api/agent/suggestions`);
    if (!res.ok) return [];
    ...
    <div className="gm">
      ...
      {loading ? (
        ...
        <div className="gm__empty">
          {filter === 'all'
            ? <>No goals yet. Click <strong>+ New Goal</strong>, say <em>"hey simple, &lt;instruction&gt;"</em>, or press <kbd>Ctrl+Win+G</kbd> after copying text.</>
            : `No ${filter} goals.`}
        </div>
      ) : (
