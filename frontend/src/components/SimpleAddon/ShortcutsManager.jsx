...
function asAddonConnectionErrorMessage(message) {
  ...
  return 'Lost connection to Simple addon. Make sure the addon is running, then try again.';
}
    ...
    try {
      ...
      throw new Error(
        addonConnected
          ...
          : 'Sign in to use cloud macro compilation, or install the Simple addon.'
      );
    } catch (e) {
  ...
  const handleEditWithNl = useCallback(async () => {
    ...
    try {
      ...
      throw new Error(
        addonConnected
          ? 'Addon routes unavailable. Quit and relaunch the addon from the system tray.'
          : 'Sign in to use cloud macro editing, or install the Simple addon.'
      );
    } catch (e) {
      ...
      <div className="short__intro">
        ...
        {!addonConnected && (
          <p className="short__banner short__banner--warn">
            ⚠ Local Simple addon not connected — recording and running macros are disabled.
          </p>
        )}
      </div>
