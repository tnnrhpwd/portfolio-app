...
import { detectAddon, getAddonStatus, onAddonStatusChange, startAddonPolling, checkRemoteAddon, onRemoteAddonStatusChange, getRemoteAddonStatus, registerCloudRelay, setAddonOptIn } from '../../services/simpleAddonApi';
...
async function fetchLatestRelease() {
  ...
  const CACHE_TS_KEY = 'simple_latest_version_ts';
...
/**
 * React hook for detecting and monitoring the Simple local addon.
 * 
 * Returns:
...
export function useAddonDetection({ pollInterval = 30000 } = {}) {
  ...
  const [dismissed, setDismissed] = useState(() => {
    try {
      return sessionStorage.getItem('simple_addon_dismissed') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const dismissPrompt = useCallback(() => {
    setDismissed(true);
    try {
      sessionStorage.setItem('simple_addon_dismissed', 'true');
    } catch {
