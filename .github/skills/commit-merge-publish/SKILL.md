...
Draft a subject (≤72 chars) + optional body explaining _why_, then commit:
...
4.2. **Ask before releasing — every time.** A release is the most expensive thing this
repo does: `build-addon.yml` runs on `windows-latest` (~3.5 min of build minutes) and
publishes a GitHub Release that installed apps auto-update from. So: never cut one to
find out whether the code works, and never cut one merely because the change touched
`simple-addon/`. A merge or a push never implies a release; only an explicit "ship it"
does.

4.3. Before releasing, run the addon's local gate and only continue if it is green:

```
npm --prefix simple-addon test
```

That is exactly what CI's addon job runs (`test:unit` **then** `eval`). A unit-only run
does not cover the eval scenarios — that is where the CI-only failures lived, so skipping
this just moves the discovery onto a paid runner.

4.4. If the user confirmed, run from `simple-addon/`:
...
4.5. Confirm the CI run picked it up:
